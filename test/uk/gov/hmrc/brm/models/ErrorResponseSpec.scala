...
class ErrorResponseSpec extends AnyWordSpecLike with Matchers with OptionValues with GuiceOneAppPerSuite with ScalaFutures {
  ...
  "ErrorResponses" should {
    ...
    "return BadRequest with specific body when firstName is invalid" in {
      ...
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "message").as[String] shouldBe InvalidFirstName.message
      response.header.status shouldBe BAD_REQUEST
    }
    ...
    "return BadRequest with specific body when lastName is invalid" in {
      ...
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "message").as[String] shouldBe InvalidLastName.message
      response.header.status shouldBe BAD_REQUEST
    }
    ...
    "return BadRequest with specific body when dateOfBirth is invalid" in {
      ...
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "code").as[String] shouldBe InvalidDateOfBirth.code
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "message").as[String] shouldBe InvalidDateOfBirth.message
      response.header.status shouldBe BAD_REQUEST
    }
    ...
    "return BadRequest with specific body when birthReferenceNumber is invalid" in {
      ...
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "code").as[String] shouldBe InvalidBirthReferenceNumber.code
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "message").as[String] shouldBe InvalidBirthReferenceNumber.message
      response.header.status shouldBe BAD_REQUEST
    }
    ...
    "return Forbidden with specific body when whereBirthRegistered is invalid" in {
      ...
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "code").as[String] shouldBe InvalidWhereBirthRegistered.code
      (Json.parse(response.body.consumeData.futureValue.utf8String) \ "message").as[String] shouldBe InvalidWhereBirthRegistered.message
