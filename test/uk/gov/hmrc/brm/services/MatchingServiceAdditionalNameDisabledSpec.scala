      ...
      references.foreach { reference =>
        ...
        "not ignore middle names with feature toggle disabled" should {
          ...
          s"($name) match true when payload has more than one additional name and record has same additional name." in {
            ...
            val resultMatch =
              testMatchingService.performMatch(payload, List(adamTestTestJonesRecord), MatchingType.FULL)
            resultMatch.matched               shouldBe true
          ...
          s"($name) match when firstname and additional name  contains special character name and record have same name." in {
            ...
            val record      = getRecord(
              "ÀÁÂÃÄÅÆÇÈÉÊËÌÍ ÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿÀÁÂÃÄÅÆÇÈÉÊËÌÍ" +
                ...
                "ûüýþÿÀÁÂÃÄÅÆÇÈÉÊËÌÍ ÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷ø",
              "joshi"
            )
            val payload     = Payload(
              reference,
          ...
          s"($name) match when firstname and additional name with very long multiple names and record have same name." in {
            mockAuditSuccess
            val record      = getRecord(
              "Henry TEST George Martin Malcolm Arthur Cameron McTavish Glenny Alberto Turton " +
            ...
            val payload     = Payload(
              ...
              "Henry",
              Some(
                "test  George Martin Malcolm Arthur Cameron  McTavish Glenny Alberto " +
